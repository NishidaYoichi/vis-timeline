...
class TimeStep {
  ...
  /**
   * Set custom formatting for the minor and middle and major labels of the TimeStep.
  ...
  getLabelMiddle(date) {
    ...
    switch (this.scale) {
      case 'week':
        // Don't draw the middle label if this date is the first day of a month AND if it's NOT the start of the week.
        // The 'date' variable may actually be the 'next' step when called from TimeAxis' _repaintLabels.
        if(date.date() === 1 && date.weekday() !== 0){
            return "";
        }
